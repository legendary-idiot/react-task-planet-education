...
const Sticker = ({ text, className }) => (
	<div
		className={`inline-flex items-center font-extrabold text-base md:text-lg px-4 py-2 rotate-[-14deg] shadow-[0_6px_0_rgba(0,0,0,0.2)] ${className}`}
	>
		{text}
	</div>
);
...
const Card = ({ campus }) => (
	<div className="group relative isolate overflow-hidden bg-white/5 backdrop-blur">
		<img
			src={campus.img}
			alt={campus.title}
			className="w-[245px] h-auto object-cover object-center transition-transform duration-500 group-hover:scale-[1.03]"
		/>
		<a href="#" className="absolute top-0 inset-0 h-10 flex items-center gap-2 sm:gap-8 py-4 px-3 md:px-4 bg-[#8EC8EE] backdrop-blur-sm hover:bg-[#8EC8EE]/60 hover:scale-[0.99]">
			<span className="text-[11px] xsm:text-[14px] md:text-base lg:text-lg font-extrabold text-[#061665] tracking-wider">
				{campus.title}</span><span className="text-[#061665] text-[12px] sm:text-lg"><FaArrowRight /></span>
		</a>
	</div>
);
...
const CampusesSection = () => {
	return (
		<section className="relative overflow-hidden bg-[#061665CC]">
			{/* Floating stickers */}
			...
			<div className="hidden lg:block absolute lg:right-4 xl:right-1/12 top-1/2">
				<Sticker text="BOROUGH" className="bg-[#00FFD2] text-[#061665] rotate-[16deg]" />
			</div>
			<div className="hidden lg:block absolute lg:left-4 xl:left-1/12 bottom-1/5">
				<Sticker text="MANCHESTER" className="bg-[#8EC8EE] rotate-[14deg] text-[#061665] " />
			</div>
			...
			<div className="relative max-w-[1200px] flex flex-col items-center mx-auto px-4 md:px-6 lg:px-8 py-16 md:py-20">
				{/* Cards grid */}
				<div className="w-fit flex gap-3 md:gap-4 relative z-[1]">
					{campuses.map((c) => (
				...
				<div className="text-center mt-10 md:mt-12">
					...
					We have multiple locations across the UK, all strategically situated in vibrant city centers and easily accessible by public transport.
					</p>
				</div>
			</div>
		</section>
